...
class Search {
    get options() {
        return {
            ...
            placeholder: "Search",
            loading: "Loading",
            jsonPath: "/assets/json/search.json",
        };
    }
    ...
    _init() {
        ...
        this._autoComplete = new autoComplete({
            ...
            noResults: () => {
                ...
                document
                    .getElementById(this.settings.searchResultsId)
                    .appendChild(result);
            },
            onSelection: (feedback) => {
                window.location.href = Helpers.UrlFix(
                    feedback.selection.value["url"]
                );
                searchInput.value = "";
    ...
    _addListeners() {
        document
            .getElementById(this.settings.searchModalId)
            .addEventListener(
                "shown.bs.modal",
                this._onSearchModalShown.bind(this)
            );
        document
            .getElementById(this.settings.searchModalId)
            .addEventListener(
                "hidden.bs.modal",
                this._onSearchModalHidden.bind(this)
            );
        document
            .getElementById(this.settings.searchInputId)
            .addEventListener("focus", this._onInputFocus.bind(this));
        document
            .getElementById(this.settings.searchInputId)
            .addEventListener("focusout", this._onInputFocusOut.bind(this));
    }
    ...
    _onInputFocusOut() {
        document
            .getElementById(this.settings.searchInputId)
            .removeEventListener("keydown", this._onKeyDown.bind(this));
    }
